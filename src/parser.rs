...
impl<'a> Parser<'a> {
    ...
    fn parse_variable(&mut self, line: &str) -> Result<()> {
        ...
        if parts.len() != 2 {
            bail!("Invalid variable assignment at line {}", self.current_line + 1);
        }
        
        let name = parts[0].trim().to_string();
        let mut value = parts[1].trim().to_string();
        
        while self.current_line + 1 < self.lines.len() && self.lines[self.current_line].ends_with('\\') {
            value.pop();
            self.current_line += 1;
    ...
    fn parse_rule(&mut self) -> Result<()> {
        ...
        while self.current_line < self.lines.len() {
            ...
            if recipe_line.starts_with('\t') || recipe_line.starts_with(' ') {
                ...
                let indentation = &recipe_line[..recipe_line.len() - recipe_line.trim_start().len()];
